...
set output "DephD_noise_1570_L.eps"
...
phi = -2.1
A=0.9
f(x) = A*2.7182818284590452353**(-(x*1e-6)/tau)*sin(2*3.14*omega*(x*1e-6) + phi)/2 +0.5

# f(x) = 2.7182818284590452353**(-1*((x*1e-6)/tau)**2/2)*sin(2*3.14*omega*(x*1e-6) + phi)/2 +0.5


fit f(x) 'DephD_noise_1570_L.txt' u ($1*1e6):5 via tau, omega, phi,A
set samples 500
set xrange [0:]
plot 'DephD_noise_1570_L.txt' u ($1*1e6):2 w lp ls 1 title '00',\
	 'DephD_noise_1570_L.txt' u ($1*1e6):3 w lp ls 2 title '01',\
	 'DephD_noise_1570_L.txt' u ($1*1e6):4 w lp ls 3 title '10',\
	 'DephD_noise_1570_L.txt' u ($1*1e6):5 w lp ls 4 title '11',\
