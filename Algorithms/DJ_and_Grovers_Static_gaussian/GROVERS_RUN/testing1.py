...
def grovers(state, show=False, samples= 1):
	...
	db.awg_pulse(detuningE/np.pi/2, 224e-9, 274e-9+a, .5e-9)
	db.awg_pulse_tc(0.25e9, 220e-9, 280e-9+a, 0.05e-9)

	db.mw_pulse(19.7e9,np.pi/2+phaseQ1,4.4e7,320e-9,434e-9)
	db.mw_pulse(18.4e9,np.pi/2+phaseQ2,4.4e7,320e-9,434e-9)

	db.awg_pulse(detuningE/np.pi/2, 443e-9, 493e-9+a, .5e-9)
	db.awg_pulse_tc(0.25e9, 438e-9, 498e-9+a, 0.05e-9)
